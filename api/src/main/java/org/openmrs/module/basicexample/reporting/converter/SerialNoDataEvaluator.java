...
@Handler(supports = SerialNoDataDefinition.class, order = 53)
public class SerialNoDataEvaluator implements PatientDataEvaluator {
	...
	public EvaluatedPatientData evaluate(PatientDataDefinition patientDataDefinition, EvaluationContext evaluationContext)
		...
		String qry = "select patient_id,pi.identifier FROM openmrs.encounter e "
		        ...
		        + "AND pit.name in('Patient Clinic Number') group by e.patient_id";
